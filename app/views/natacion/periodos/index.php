<?php
/**
 * DigiSports Natación - Períodos Académicos
 */
$periodos    = $periodos ?? [];
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><strong><?= htmlspecialchars($p['npe_nombre']) ?></strong>
                                    <?php if (!empty($p['npe_notas'])): ?><br><small class="text-muted"><?= htmlspecialchars(substr($p['npe_notas'], 0, 60)) ?></small><?php endif; ?>
                                </td>
                                <td><?= date('d/m/Y', strtotime($p['npe_fecha_inicio'])) ?></td>
                                <td><?= date('d/m/Y', strtotime($p['npe_fecha_fin'])) ?></td>
                                ...
                                <td class="text-center">
                                    <?php $eb = ['PLANIFICADO'=>'secondary','ACTIVO'=>'success','FINALIZADO'=>'dark','CANCELADO'=>'danger'][$p['npe_estado']] ?? 'light'; ?>
                                    <span class="badge badge-<?= $eb ?>"><?= $p['npe_estado'] ?></span>
                                </td>
                                <td class="text-center">
                                    <div class="btn-group btn-group-sm">
                                        <button class="btn btn-outline-primary" onclick='editarPeriodo(<?= json_encode($p) ?>)' title="Editar"><i class="fas fa-edit"></i></button>
                                        <button class="btn btn-outline-danger" onclick="finalizarPeriodo(<?= $p['npe_periodo_id'] ?>,'<?= htmlspecialchars($p['npe_nombre']) ?>')" title="Finalizar"><i class="fas fa-flag-checkered"></i></button>
                                    </div>
                                </td>
                            </tr>
                            <?php endforeach; ?>
...
<?php ob_start(); ?>
<script>
var urlCrear = '<?= url('natacion', 'periodo', 'crear') ?>';
var urlEditar = '<?= url('natacion', 'periodo', 'editar') ?>';
function abrirModal() {
...
function editarPeriodo(p) {
    document.getElementById('per_id').value = p.npe_periodo_id;
    document.getElementById('per_nombre').value = p.npe_nombre || '';
    document.getElementById('per_inicio').value = p.npe_fecha_inicio || '';
    document.getElementById('per_fin').value = p.npe_fecha_fin || '';
    document.getElementById('per_estado').value = p.npe_estado || 'PLANIFICADO';
    document.getElementById('per_notas').value = p.npe_notas || '';
...
function finalizarPeriodo(id, nombre) {
    ...
    }).then(function(r) { if (r.isConfirmed) window.location.href = '<?= url('natacion', 'periodo', 'eliminar') ?>&id=' + id; });
}
</script>
